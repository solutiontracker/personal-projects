...
class RegistrationInvitationReminderLog extends Component {
    _isMounted = false;
    ...
    listing = (activePage = 1, loader = false) => {
        ...
        service
            .post(`${process.env.REACT_APP_URL}/attendee/registration/invitations/reminder-log/${activePage}`, this.state)
            .then(
                response => {
                    if (response.success) {
                        if (this._isMounted) {
                            ...
                            <Translation key={key}>
                                {
                                    t =>
                                        <div className={this.state.editElement && this.state.editElement && this.state.editElementIndex === key ? "no-hover row d-flex" : "row d-flex"} key={key}>
                                            ...
                                            <div className="col-3">
                                                {data.date_sent && <p>{data.date_sent}</p>}
                                            </div>
                                        </div>
                                }
                            </Translation>
                            ...
                            {!this.state.preLoader && (
                                <React.Fragment>
                                    ...
                                    <div className="bottom-component-panel clearfix">
                                        ...
                                        <NavLink className="btn btn-prev-step" to={`/event/invitation/report/not-attendees-list`}><span className="material-icons">
                                            keyboard_backspace</span></NavLink>
                                        <NavLink className="btn btn-next-step" to={`/event/invitation/report/registration/settings`}>{t('G_NEXT')}</NavLink>
                                    </div>
                                </React.Fragment>
                            )}
                        </div>
...
export default RegistrationInvitationReminderLog;
