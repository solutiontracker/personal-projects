...
class Page extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            pdf: '',
            type: 'page',
            page_type: this.props.cms === 'information-pages' ? '2' : '1',
            menu_id: '0',
            parent_id: '0',
            image_position:'top',
            validate_name: '',
    ...
    componentDidMount() {
        if (this.props.editData) {
            this.setState({
                ...
                menu_id: this.props.editData.section_id,
                image: this.props.editData.image,
                pdf: this.props.editData.pdf,
                validate_name: 'success',
                validate_description: 'success',
                loaded: true,
                image_position:this.props.editData.image_position,
                pdf_title:this.props.editData.detail.pdf_title,
            })
        } else {
            this.setState({
                parent_id: (this.props.parent_id !== undefined ? this.props.parent_id : 0),
                menu_id: (this.props.menu_id !== undefined ? this.props.menu_id :   (this.props.parent_id !== undefined ? this.props.parent_id : 0)),
                loaded: true
            })
        }
    }
                            ...
                            <div className="row">
                                ...
                                <div className="col-4">
                                    <div className='d-flex justify-content-start px-3 py-1'>
                                        <label className='m-0 mr-2'>Show on top</label>
                                        <input type="checkbox" name="show-in-app" checked={(this.state.image_position === 'top')} onClick={() => this.setState({image_position: 'top'})}/>
                                        <label className='m-0 ml-2 mr-2'>Show on bottom</label>
                                        <input type="checkbox" name="show-in-website" checked={(this.state.image_position === 'bottom')} onClick={() => this.setState({image_position: 'bottom'})}/>
                                    </div>
                                    <FileUpload
                                    ...
                                    <Input
                                        className={this.state.validate_pdf_title}
                                        type='text'
                                        label={'Pdf title'}
                                        name='name'
                                        value={this.state.pdf_title}
                                        onChange={this.handleChange('pdf_title', 'validate_pdf_title', 'text')}
                                        required={false}
                                    />
                                    <FileUpload
