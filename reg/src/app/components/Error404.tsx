import React, { ReactElement, FC, useContext } from 'react';
import { useHistory } from 'react-router-dom';
import { EventContext } from "@/src//app/context/event/EventProvider";

type Props = Record<string, never>;

const _styles = `body{background: #fff !important}.main-section{padding-top: 0 !important}.main-section .master-container{height: 100%}`

const Error404: FC<Props> = (): ReactElement => {

  const history = useHistory();

  const { event } = useContext<any>(EventContext);

  return (
    <main role="main" className="main-section">
      <style dangerouslySetInnerHTML={{ __html: _styles }} />
      <div id="ebs-404-page">
        <div className="container">
          <div className="row d-flex align-items-center">
            <div className="col-md-7">
              <div className="ebs-error-content">
                <h4>404 Error</h4>
                <h1>Page not found…</h1>
                <p>Sorry, we cannot find what you are looking for.
                  For further queries please contact the event organiser or send an e-mail with URL and details to  <a href="mailto:support@eventbuizz.com">support@eventbuizz.com</a>
                </p>
                {Object.keys(event).length > 0 && (
                  <a href="#!" className='ebs-btn-back' onClick={() => {
                    history.push(`/${event.url}/attendee`);
                  }}>Return</a>
                )}
              </div>
            </div>
            <div className="col-md-5 d-none d-md-block">
              <img src={require('@/src/img/404.jpg')} alt="" />
            </div>
          </div>
        </div>
      </div>
    </main>
  );

};

export default Error404;