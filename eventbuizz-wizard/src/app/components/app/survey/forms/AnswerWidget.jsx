...
export default class AnswerWidget extends Component {
                                                ...
                                                this.setState({
                                                    ...
                                                    if (option_id !== undefined) {
                                                        if (this.state.column) {
                                                            SurveyService.destroy(this.state, option_id, 'option_matrix');
                                                        }
                                                        else {
                                                            SurveyService.destroy(this.state, option_id, 'option');
                                                        }
                                                    }
                                                });
                                            }}
    ...
    handleToggle = index => e => {
        e.preventDefault();
        var answer = [...this.state.answer];
        answer[index].correct = answer[index].correct === "1" ? "0" : "1";
        this.setState({
            answer: answer
        })
        setTimeout(() => { this.props.onChange(this.state.answer); }, 50)
                                        ...
                                        {(provided, snapshot) => (
                                            ...
                                            >
                                                    ...
                                                    <Draggable key={key} draggableId={`item-${key}`} index={key}>
                                                        {(provided, snapshot) => (
                                                            ...
                                                            >
                                                                ...
                                                                <div className="list">
                                                                    {this.props.question_type !== 'matrix' &&
                                                                        <span onClick={this.handleToggle(key)}
                                                                            className="checkbox"><i className="material-icons">{item.correct === "1" ? 'check_box' : 'check_box_outline_blank'}</i> {t('ES_CORRECT')}
                                                                        </span>
                                                                    }
                                                                    <input
                                                                        onChange={this.updateOptionElement.bind(this)}
                                                                        data-index={key} type="text"
                                                                        placeholder={(this.props.column !== undefined && this.props.column === true) ? t('ES_ADD_COLUMN') : t('ES_ADD_ANSWER')} value={item.value} />
                                                ...
                                                <button onClick={this.addOptionElement.bind(this)}
                                                    className="btn btn_addquestion">{(this.props.column !== undefined && this.props.column === true) ? t('ES_ADD_A_COLUMN') : t('ES_ADD_AN_OPTION')}
                                                </button>
                                            </div>
                                        )}
                                    </Droppable>
