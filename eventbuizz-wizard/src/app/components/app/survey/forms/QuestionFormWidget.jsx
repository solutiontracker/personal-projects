...
import AnswerWidget from '@/app/survey/forms/AnswerWidget';
...
class FormWidget extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            is_anonymous: (this.props.editData ? this.props.editData.is_anonymous : '0'),
            ...
            charts: [
                { name: this.props.t('ES_PIE_CHART'), id: "pie" },
                { name: this.props.t('ES_HT_CHART'), id: 'horizontal' },
                { name: this.props.t('ES_VB_CHART'), id: 'vertical' },
                { name: this.props.t('ES_PB_CHART'), id: 'progress' }
            ],
            result_chart_type: (this.props.editData && this.props.editData.result_chart_type ? this.props.editData.result_chart_type : 'pie'),
            change: false
        }
    }

    handleChange = input => e => {
        if (input === 'question_type' || input === 'result_chart_type') {
            this.setState({
                [input]: e.value,
                change: true
            })
    ...
    render() {
        return (
            <Translation>
                {
                    t =>
                        <div className={`option-wrapper answer-widget subregistrationwrapper ${this.props.editData ? 'isGray' : ''}`}>
                            ...
                            <div className="row">
                                <div className="col-6">
                                    <DropDown
                                        label={t('ES_SELECT_QUESTION_TYPE')}
                                        listitems={[{ name: t('ES_OPTION_1'), id: "single" },
                                        { name: t('ES_OPTION_2'), id: 'multiple' },
                                        { name: t('ES_OPTION_3'), id: 'open' },
                                        { name: t('ES_OPTION_4'), id: 'number' },
                                        { name: t('ES_OPTION_5'), id: 'date' },
                                        { name: t('ES_OPTION_6'), id: 'date_time' },
                                        { name: t('ES_OPTION_8'), id: 'matrix' },
                                        { name: t('ES_OPTION_7'), id: 'dropdown' }]}
                                        selected={this.state.question_type}
                                        selectedlabel={this.getSelectedLabel([{ name: t('ES_OPTION_1'), id: "single" },
                                        { name: t('ES_OPTION_2'), id: 'multiple' },
                                        { name: t('ES_OPTION_3'), id: 'open' },
                                        { name: t('ES_OPTION_4'), id: 'number' },
                                        { name: t('ES_OPTION_5'), id: 'date' },
                                        { name: t('ES_OPTION_6'), id: 'date_time' },
                                        { name: t('ES_OPTION_8'), id: 'matrix' },
                                        { name: t('ES_OPTION_7'), id: 'dropdown' }], this.state.question_type)}
                            ...
                            <div className="row">
                                <div className="col-12">
                                    {this.state.question_type === "multiple" && (
                                        <ul style={{ paddingLeft: '88px', margin: '0px 0 6px 0' }} className="d-inline-block question-other-option">
                                                                             
                                            <li style={{ width: '100%', marginTop: '0px', maxWidth: '358px' }} className="d-inline-block">
                                                <Input
                                                    type='text'
                                    ...
                                    <ul style={this.state.question_type === "matrix" ? { paddingLeft: '24px', margin: '2px 0 6px 0' } : { paddingLeft: '88px', margin: '2px 0 6px 0' }} className="d-inline-block question-other-option">
                                    
                                        {/* <li style={{ width: '100%', marginTop: '13px', maxWidth: '358px', display: 'none' }} className="d-inline-block">
                                            <DropDown
                                                label={t('ES_RESULT_CHART_TYPE')}
                                                listitems={this.state.charts}
                                                selected={this.state.result_chart_type}
                                                selectedlabel={this.getSelectedLabel(this.state.charts, this.state.result_chart_type)}
                                                onChange={this.handleChange('result_chart_type')}
                                                required={true}
                                            />
                                        </li> */}
                                        <li className="d-inline-block"><label
                                            onClick={this.updateFlag('required_question')}><i className="material-icons">
                                                {Number(this.state.required_question) === 1 ? 'check_box' : 'check_box_outline_blank'}
                                            </i>{t('ES_REQUIRED_QUESTION')}</label></li>
                                        <li className="d-inline-block"><label
                                            onClick={this.updateFlag('enable_comments')}><i className="material-icons">
                                                {Number(this.state.enable_comments) === 1 ? 'check_box' : 'check_box_outline_blank'}
                                            </i>{t('ES_ENABLE_COMMENTS')}</label></li>
                                        <li className="d-inline-block"><label
                                            onClick={this.updateFlag('is_anonymous')}><i className="material-icons">
                                                {Number(this.state.is_anonymous) === 1 ? 'check_box' : 'check_box_outline_blank'}
                                            </i>{t('ES_ANONYMOUS')}</label></li>
                                    </ul>
                                </div>
                            </div>
