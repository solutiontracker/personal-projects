...
import Link from "@/app/event-information/formsInfoPages/Link";
import Folder from "@/app/event-information/formsInfoPages/Folder";
import Page from "@/app/event-information/formsInfoPages/Page";
import Child from "@/app/event-information/formsInfoPages/Child";
  ...
  for (let i = 0; i < path.length; i++) {
    ...
    if (i === path.length - 2) {
      const reorderedSubItems = reorder(
        (obj.subMenuItems !== undefined ? obj.subMenuItems : obj.subItems),
        sourceIndex,
        destIndex
      );
      obj.subItems = reorderedSubItems;
      if(obj.subMenuItems !== undefined){
        obj.subMenuItems = reorderedSubItems;
      }
      return obj.subItems;
    }
  }
...
class InformationPages extends React.Component {
  _isMounted = false;
  ...
  updateOrder = (items) => {
    InformationService.updateOrderInfoPages(this.state, items)
      .then(
        response => {
          if (response.success) {
            this.listing(true);
  ...
  handleshowInner = (input, index, parent_id, menu_id) => e => {
    e.preventDefault();
    this.setState({
      [input]: true,
      displayPanel: false,
  ...
  removeItem = (id, type, mainSection) => e => {
    confirmAlert({
      customUI: ({ onClose }) => {
        return (
          <Translation>
                ...
                <div className='app-main-popup'>
                  ...
                  <div className="app-footer">
                    ...
                    <button className="btn btn-success"
                      onClick={() => {
                        onClose();
                        InformationService.destroyInfoPage(this.state, id, type, mainSection)
                          .then(
                            response => {
                              if (response.success) {
                                this.listing();
                ...
                <React.Fragment>
                  <div style={{ height: '100%' }}>
                    <div style={{ margin: '0' }} className="new-header clearfix">
                      <div className="row">
                        <div className="col-6">
                          <h1 className="section-title">{(t('INFORMATION_PAGES_PAGE_TITLE'))}</h1>
                                    ...
                                    <div className="practical-data-list">
                                      ...
                                      {this.state.displayPanel && (
                                        <div className="practical-edit-panel">
                                          ...
                                          <span onClick={this.removeItem(item.id, item.type, (item.section_id === undefined))}
                                          ...
                                          {item.type === 'folder' && (
                                            <div className="wrapp_add_button">
                                              ...
                                              <div className="drop_down_panel">
                                                <button
                                                  onClick={this.handleshowInner('folder', item.id, item.section_id === undefined ? 0 : item.id, item.id)}
                                                  className="btn"><i className="icons"><Img
                                                    src={require("img/ico-addfolder-lg.svg")} /></i>{t('AI_ADD_FOLDER')}
                                                </button>
                                                <button
                                                  onClick={this.handleshowInner('link', item.id, item.section_id === undefined ? 0 : item.id, item.id)}
                                                  className="btn"><i className="icons"><Img
                                                    src={require("img/ico-addlink-lg.svg")} /></i>{t('AI_ADD_LINK')}
                                                </button>
                                                <button
                                                  onClick={this.handleshowInner('page', item.id, item.section_id === undefined ? 0 : item.id, item.id)}
                                                  className="btn"><i className="icons"><Img
