...
class AppInvitationNotSent extends Component {
    _isMounted = false;
    ...
    listing = (activePage = 1, loader = false) => {
        ...
        service
            .post(`${process.env.REACT_APP_URL}/attendee/app-invitations-not-sent/${activePage}`, this.state)
            .then(
                response => {
                    if (response.success) {
                        if (this._isMounted) {
    ...
    sendAction = (action, invite_type) => e => {
        ...
        this.props.dispatch(
            EventAction.invitation({
                ids: ids,
                action: action,
                module: 'app_invitation_not_sent',
                            ...
                            {!this.state.preLoader && (
                                <React.Fragment>
                                    ...
                                    <header className="new-header clearfix">
                                        ...
                                        >
                                            {t("ATTENDEE_APP_INVITATION_NOT_SENT")}
                                        </h1>
                                    ...
                                    <div className="bottom-component-panel clearfix">
                                        ...
                                        <NavLink className="btn btn-prev-step" to={`/event/invitation/report/app-invitation`}><span className="material-icons">
                                            keyboard_backspace</span></NavLink>
                                        <NavLink className="btn btn-next-step" to={`/event/invitation/report/app-reminder-log`}>{t('G_NEXT')}</NavLink>
                                    </div>
                                </React.Fragment>
                            )}
                        </div>
...
export default connect(mapStateToProps)(AppInvitationNotSent);
