...
class EventModuleOrder extends React.Component {
    _isMounted = false;
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            next: "/event/module/eventsite-module-order",
    ...
    componentDidMount() {
        ...
        //set next previous
        if (this.props.event.modules !== undefined && this.props.event.modules.length > 0 && Number(this.props.event.is_registration) === 0) {
    ...
    modules = () => {
        ...
        service.post(`${process.env.REACT_APP_URL}/event-settings/modules`, this.state)
            .then(
                response => {
                    if (response.success) {
                        if (this._isMounted) {
            ...
            .then(
                response => {
                    if (response.success) {
                        ...
                        this.props.event.defaultDirectory = response.data.defaultDirectory;
                        this.props.event.directory_sub_modules = response.data.directory_sub_modules;
                        ...
                        <div className="wrapper-content third-step main-landing-page">
                            ...
                            {!this.state.preLoader && (
                                <React.Fragment>
                                    ...
                                    <h1 className="section-title">{t('ED_CUSTOMIZE_APP_MENUS')}</h1>
                                    <div className="row d-flex">
                                        <div className="col-6">
                                            <div data-count="3" className="customize-menu counter-wrapp">
                                                <p>{t('ED_CUSTOMIZE_MENUS_DETAIL')}</p>
                                                <h4>{t('ED_APP_MENU')} <em className="app-tooltip"><i className="material-icons">info</i>
                                                                ...
                                                                <Droppable droppableId="droppable">
                                                                    {(provided, snapshot) => (
                                                                        <div
                                                                            ...
                                                                            {this.state.modules.length && this.state.modules.map((item, index) => (
                                                                                <Draggable
                                                                                    key={`item-${item.id}`}
                                                                                    ...
                                                                                    {(provided, snapshot) => (
                                                                                        <div
                                                                                            onClick={this.handleSelect(index, 'modules')}
                                                                                            ...
                                                                                            <div className="inner-wrapper">
                                                                                                <input type="text" onClick={(e) => e.stopPropagation()}
                                                                                                    onChange={this.handleInput(index, 'modules')}
                                                                                                    value={item.value}
                                                                                                    disabled={this.state.editIndex === index && this.state.activeState === 'modules' ? false : true} />
                                                                                            </div>
                                                                                            <span onClick={this.handleEdit(index, 'modules')}
                                                                                                className="list-edit material-icons">{this.state.editIndex === index && this.state.activeState === 'modules' ? 'check' : 'edit'}</span>
                                                                                        </div>
                                                                                    )}
                                                                                </Draggable>
                                                                            ))}
...
export default connect(mapStateToProps)(EventModuleOrder);