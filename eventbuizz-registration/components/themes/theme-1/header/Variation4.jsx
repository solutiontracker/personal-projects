...
class Variation4 extends React.Component {
  ...
  async componentDidMount() {
    ...
    const top_menu_length = this.state.topMenu.length / 2
    const rows = this.state.topMenu.reduce(function (rows, key, index) {
      return (
        (index % top_menu_length === 0
          ? rows.push([key])
  ...
  render() {
    const { menus, event, menuresponsive, topMenu } = this.state;
