...
const CookiePolicy: FC<any> = (): ReactElement => {
  ...
  const { updateCookie, cookie, event } = useContext<any>(EventContext);
  ...
  return (
    <React.Fragment>
      ...
      {(show || (!cookie && event?.id !== undefined)) && <div className="ebs-cookie-container">
        {cookie && (
        ...
        <p>We use cookies: You can always revoke your consent by blocking the use of cookies in your browser.
          <a target={'_blank'} rel="noreferrer" className="ebs-logo" href={`/${event.url}/attendee/cookie-policy`}>
            More info
          </a>
        </p>
        <div className="ebs-cookie-type">
          <label className="label-radio">
            <input type="checkbox" name="cookie" checked={true} />
            <span>Necessary</span>
          </label>
          <label className="label-radio" onClick={(e: any) => {
            e.preventDefault();
            e.stopPropagation();
            updateCookie(cookie === 'all' ? 'necessary' : 'all');
          }}>
            <input type="checkbox" name="cookie" checked={cookie === 'all'} />
        ...
        <button className="btn bordered" onClick={(e: any) => {
          e.preventDefault();
          e.stopPropagation();
          updateCookie('necessary');
        }}>Reject cookies</button>
      </div>}
      {!cookie && event?.id !== undefined && (
        <div className="site-blocker"></div>
      )}
    </React.Fragment>
  );
};
