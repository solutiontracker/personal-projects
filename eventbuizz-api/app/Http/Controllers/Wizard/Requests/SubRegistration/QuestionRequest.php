...
namespace App\Http\Controllers\Wizard\Requests\SubRegistration;
    ...
    protected function prepareForValidation()
    {
        ...
        if (in_array($this->request->get('question_type'), ['single', 'multiple', 'dropdown', 'matrix'])) {
            if($this->request->has('answer')) {
                $answers = (\Route::is('wizard-sub-registration-question-update') ? $this->request->get('answer') : json_decode($this->request->get('answer'), true));
            ...
            if($this->request->get('question_type') == 'matrix') {
                $column  = 0;
                $answers = (\Route::is('wizard-sub-registration-question-update') ? $this->request->get('column') : json_decode($this->request->get('column'), true));
                foreach ($answers as $key => $val) {
